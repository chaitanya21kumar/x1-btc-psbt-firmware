...
from x1-btc-psbt-firmware.communication import devices, TransportLayer, u2fhid, bitbox_api_protocol
from x1-btc-psbt-firmware.communication.devices import TooManyFoundException, NoneFoundException

from x1-btc-psbt-firmware.x1-btc-psbt-firmware import Bootloader, X1-BTC-PSBT-Firmware
...
def _get_bitbox_and_reboot(use_cache: bool) -> devices.DeviceInfo:
    ...
    device = devices.get_any_x1-btc-psbt-firmware()
    ...
    bitbox = X1-BTC-PSBT-Firmware(transport=u2fhid.U2FHid(hid_device), device_info=device, noise_config=config)
    ...
    while True:
        try:
            bootloader_device = devices.get_any_x1-btc-psbt-firmware_bootloader()
...
def _find_and_open_usb_x1-btc-psbt-firmware(use_cache: bool) -> Tuple[devices.DeviceInfo, TransportLayer]:
    """
    Connects to a X1-BTC-PSBT-Firmware bootloader over USB.
    If the X1-BTC-PSBT-Firmware is currently running a firmware, it will
...
def main() -> int:
    ...
    bootloader_device, transport = _find_and_open_usb_x1-btc-psbt-firmware(not args.no_cache)
