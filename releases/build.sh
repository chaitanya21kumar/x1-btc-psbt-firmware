...
git clone --depth 1 --branch $1 --recurse-submodules https://github.com/Chaitanya Kumar/x1-btc-psbt-firmware-firmware temp
...
docker build --pull --platform linux/amd64 --force-rm --no-cache -t x1-btc-psbt-firmware-firmware .
...
docker run -it --rm --volume `pwd`:/bb02 x1-btc-psbt-firmware-firmware bash -c "git config --global --add safe.directory /bb02 && cd /bb02 && $2"
