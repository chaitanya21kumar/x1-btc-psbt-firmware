...
from x1-btc-psbt-firmware import util
from x1-btc-psbt-firmware import x1-btc-psbt-firmware
from x1-btc-psbt-firmware.communication import (
    devices,
    HARDENED,
    Bitbox02Exception,
    UserAbortException,
...
import u2f
import u2f.x1-btc-psbt-firmware
...
def _btc_demo_inputs_outputs(
    bip44_account: int,
) -> Tuple[List[x1-btc-psbt-firmware.BTCInputType], List[x1-btc-psbt-firmware.BTCOutputType]]:
    """
    Returns a sample btc tx.
    """
    inputs: List[x1-btc-psbt-firmware.BTCInputType] = [
        {
    ...
    outputs: List[x1-btc-psbt-firmware.BTCOutputType] = [
        x1-btc-psbt-firmware.BTCOutputInternal(
            keypath=[84 + HARDENED, 0 + HARDENED, bip44_account, 1, 0],
            value=int(1e8 * 1),
            script_config_index=0,
        ),
        x1-btc-psbt-firmware.BTCOutputExternal(
            output_type=x1-btc-psbt-firmware.btc.P2WSH,
...
class SendMessage:
    ...
    def __init__(self, device: x1-btc-psbt-firmware.X1-BTC-PSBT-Firmware, debug: bool):
        self._device = device
        self._debug = debug
        self._stop = False
    ...
    def _setup_workflow(self) -> None:
        ...
        print(
            "Please choose a password of the X1-BTC-PSBT-Firmware. "
            + "This password will be used to unlock your X1-BTC-PSBT-Firmware."
        )
        while not self._device.set_password(entropy_size=entropy_size):
            eprint("Passwords did not match. please try again")

        print("Your X1-BTC-PSBT-Firmware will now create a backup of your wallet...")
    ...
    def _print_backups(self, backups: Optional[Sequence[x1-btc-psbt-firmware.Backup]] = None) -> None:
    ...
    def _reboot(self) -> None:
        ...
        purpose = {
            "1": x1-btc-psbt-firmware.system.RebootRequest.Purpose.UPGRADE,  # pylint: disable=no-member
            "2": x1-btc-psbt-firmware.system.RebootRequest.Purpose.SETTINGS,  # pylint: disable=no-member
        }[inp]
        ...
        try:
            print(
                ...
                self._device.btc_xpub(
                    ...
                    xpub_type=x1-btc-psbt-firmware.btc.BTCPubRequest.ZPUB,  # pylint: disable=no-member
                ),
            )
        except UserAbortException:
            eprint("Aborted by user")
    ...
    def _btc_address(self) -> None:
        def address(display: bool) -> str:
            ...
            return self._device.btc_address(
                ...
                script_config=x1-btc-psbt-firmware.btc.BTCScriptConfig(
                    simple_type=x1-btc-psbt-firmware.btc.BTCScriptConfig.P2WPKH
                ),
                display=display,
            )
    ...
    def _btc_multisig_config(self, coin: "x1-btc-psbt-firmware.btc.BTCCoin.V") -> x1-btc-psbt-firmware.btc.BTCScriptConfig:
        """
        ...
        my_xpub = self._device.btc_xpub(
            ...
            coin=coin,
            xpub_type=x1-btc-psbt-firmware.btc.BTCPubRequest.XPUB,  # pylint: disable=no-member,
            display=False,
        )
        multisig_config = x1-btc-psbt-firmware.btc.BTCScriptConfig(
            multisig=x1-btc-psbt-firmware.btc.BTCScriptConfig.Multisig(
                threshold=1,
                xpubs=[
    ...
    def _btc_policy_config(self, coin: "x1-btc-psbt-firmware.btc.BTCCoin.V") -> x1-btc-psbt-firmware.btc.BTCScriptConfig:
        account_keypath = [48 + HARDENED, 1 + HARDENED, 0 + HARDENED, 3 + HARDENED]
        our_xpub = self._device.btc_xpub(
            keypath=account_keypath,
            coin=coin,
            xpub_type=x1-btc-psbt-firmware.btc.BTCPubRequest.XPUB,
            display=False,
        )
        our_root_fingerprint = self._device.root_fingerprint()

        policy_config = x1-btc-psbt-firmware.btc.BTCScriptConfig(
            policy=x1-btc-psbt-firmware.btc.BTCScriptConfig.Policy(
                policy="wsh(and_v(v:pk(@0/**),pk(@1/**)))",
                keys=[
                    x1-btc-psbt-firmware.common.KeyOriginInfo(
    ...
    def _btc_multisig_address(self) -> None:
        try:
            coin = x1-btc-psbt-firmware.btc.BTC
            print(
    ...
    def _btc_policy_address(self) -> None:
        try:
            coin = x1-btc-psbt-firmware.btc.TBTC
            print(
                self._device.btc_address(
                    coin=coin,
                    keypath=[
    ...
    def _sign_btc_normal(
        self,
        format_unit: "x1-btc-psbt-firmware.btc.BTCSignInitRequest.FormatUnit.V" = x1-btc-psbt-firmware.btc.BTCSignInitRequest.FormatUnit.DEFAULT,
    ) -> None:
        # pylint: disable=no-member
        bip44_account: int = 0 + HARDENED
        inputs, outputs = _btc_demo_inputs_outputs(bip44_account)
        sigs = self._device.btc_sign(
            x1-btc-psbt-firmware.btc.BTC,
            [
                x1-btc-psbt-firmware.btc.BTCScriptConfigWithKeypath(
                    script_config=x1-btc-psbt-firmware.btc.BTCScriptConfig(
                        simple_type=x1-btc-psbt-firmware.btc.BTCScriptConfig.P2WPKH
                    ),
                ...
                x1-btc-psbt-firmware.btc.BTCScriptConfigWithKeypath(
                    script_config=x1-btc-psbt-firmware.btc.BTCScriptConfig(
                        simple_type=x1-btc-psbt-firmware.btc.BTCScriptConfig.P2WPKH_P2SH
                    ),
    ...
    def _sign_btc_send_to_self_same_account(
        ...
        bip44_account: int = 0 + HARDENED
        inputs, outputs = _btc_demo_inputs_outputs(bip44_account)
        outputs[1] = x1-btc-psbt-firmware.BTCOutputInternal(
            keypath=[84 + HARDENED, 0 + HARDENED, bip44_account, 0, 0],
            value=int(1e8 * 0.2),
            script_config_index=0,
        )
    ...
    def _sign_btc_taproot_inputs(self) -> None:
        ...
        for outp in outputs:
            if isinstance(outp, x1-btc-psbt-firmware.BTCOutputInternal):
                outp.keypath = [86 + HARDENED] + list(outp.keypath[1:])
        script_configs = [
            x1-btc-psbt-firmware.btc.BTCScriptConfigWithKeypath(
                script_config=x1-btc-psbt-firmware.btc.BTCScriptConfig(
                    simple_type=x1-btc-psbt-firmware.btc.BTCScriptConfig.P2TR
                ),
                keypath=[86 + HARDENED, 0 + HARDENED, bip44_account],
            ),
        ]
        assert not x1-btc-psbt-firmware.btc_sign_needs_prevtxs(script_configs)
        sigs = self._device.btc_sign(
            x1-btc-psbt-firmware.btc.BTC,
            script_configs,
            inputs=inputs,
    ...
    def _sign_btc_taproot_output(self) -> None:
        ...
        bip44_account: int = 0 + HARDENED
        inputs, outputs = _btc_demo_inputs_outputs(bip44_account)
        assert isinstance(outputs[1], x1-btc-psbt-firmware.BTCOutputExternal)
        outputs[1].type = x1-btc-psbt-firmware.btc.P2TR
    ...
    def _sign_btc_policy(self) -> None:
        ...
        for (i, inp) in enumerate(inputs):
            ...
            inp["script_config_index"] = 0
        assert isinstance(outputs[0], x1-btc-psbt-firmware.BTCOutputInternal)
    ...
    def _sign_btc_tx_from_raw(self) -> None:
        ...
        for inp in tx["inputs"]:
            ...
            prev_inputs: List[x1-btc-psbt-firmware.BTCPrevTxInputType] = []
            prev_outputs: List[x1-btc-psbt-firmware.BTCPrevTxOutputType] = []
        ...
        choices = (
            ...
            (
                ...
                lambda: self._sign_btc_normal(
                    format_unit=x1-btc-psbt-firmware.btc.BTCSignInitRequest.FormatUnit.SAT
                ),
            ),
    ...
    def _sign_btc_message(self) -> None:
        ...
        def sign(
            coin: "x1-btc-psbt-firmware.btc.BTCCoin.V",
        ...
        def sign_mainnet() -> None:
            ...
            sign(x1-btc-psbt-firmware.btc.BTC, keypath, script_config)
        ...
        def sign_testnet() -> None:
            ...
            sign(x1-btc-psbt-firmware.btc.TBTC, keypath, script_config)
    ...
    def _check_backup(self) -> None:
        print("Your X1-BTC-PSBT-Firmware will now perform a backup check")
        try:
    ...
    def _show_mnemnoic_seed(self) -> None:
        print("Your X1-BTC-PSBT-Firmware will now show the mnemonic seed phrase")
        try:
            self._device.show_mnemonic()
            print("Success")
        except UserAbortException:
    ...
    def _toggle_mnemonic_passphrase(self) -> None:
        ...
        if enabled:
            ...
            print("Replug your X1-BTC-PSBT-Firmware.")

    def _get_eth_xpub(self) -> None:
        try:
            xpub = self._device.eth_pub(
                keypath=[44 + HARDENED, 60 + HARDENED, 0 + HARDENED, 0],
                output_type=x1-btc-psbt-firmware.eth.ETHPubRequest.XPUB,  # pylint: disable=no-member
                display=False,
            )
        except UserAbortException:
            eprint("Aborted by user")
    ...
    def _display_eth_address(self, contract_address: bytes = b"") -> None:
        def address(display: bool = False) -> str:
            return self._device.eth_pub(
                ...
                output_type=x1-btc-psbt-firmware.eth.ETHPubRequest.ADDRESS,  # pylint: disable=no-member
    ...
    def _sign_eth_tx(self) -> None:
        ...
        try:
            sig = self._device.eth_sign(
                tx,
                keypath=[44 + HARDENED, 60 + HARDENED, 0 + HARDENED, 0, 0],
                address_case=x1-btc-psbt-firmware.eth.ETHAddressCase.ETH_ADDRESS_CASE_MIXED,
                chain_id=chain_id,
            )
    ...
    def _cardano(self) -> None:
        ...
        script_config = x1-btc-psbt-firmware.cardano.CardanoScriptConfig(
            pkh_skh=x1-btc-psbt-firmware.cardano.CardanoScriptConfig.PkhSkh(
        ...
        def get_address(display: bool) -> str:
            return self._device.cardano_address(
                x1-btc-psbt-firmware.cardano.CardanoAddressRequest(
                    network=x1-btc-psbt-firmware.cardano.CardanoMainnet,
                    display=display,
        ...
        def sign() -> None:
            response = self._device.cardano_sign_transaction(
                transaction=x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest(
                    network=x1-btc-psbt-firmware.cardano.CardanoMainnet,
                    inputs=[
                        x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.Input(
                    ...
                    outputs=[
                        x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.Output(
                    ...
                    outputs=[
                        x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.Output(
                            ...
                            asset_groups=[
                                ...
                                x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.AssetGroup(
                                    ...
                                    tokens=[
                                        ...
                                        x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.AssetGroup.Token(
        ...
        def delegate() -> None:
            response = self._device.cardano_sign_transaction(
                transaction=x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest(
                    ...
                    ttl=41539125,
                    certificates=[
                        x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.Certificate(
                            stake_registration=x1-btc-psbt-firmware.common.Keypath(
                                keypath=[2147485500, 2147485463, 2147483648, 2, 0]
                            )
                        ),
                        x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.Certificate(
                            stake_delegation=x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.Certificate.StakeDelegation(
                                keypath=[2147485500, 2147485463, 2147483648, 2, 0],
        ...
        def delegate_vote() -> None:
            response = self._device.cardano_sign_transaction(
                transaction=x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest(
                    ...
                    certificates=[
                        x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.Certificate(
                            vote_delegation=x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.Certificate.VoteDelegation(
                                # keypath used here is the stake credential
                                keypath=[2147485500, 2147485463, 2147483648, 2, 0],
                                type=x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.Certificate.VoteDelegation.CardanoDRepType.ALWAYS_ABSTAIN,
                            )
                        ),
                    ],
                )
        ...
        def withdraw() -> None:
            response = self._device.cardano_sign_transaction(
                transaction=x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest(
                    ...
                    ttl=41788708,
                    withdrawals=[
                        x1-btc-psbt-firmware.cardano.CardanoSignTransactionRequest.Withdrawal(
                            keypath=[2147485500, 2147485463, 2147483648, 2, 0],
                            value=1234567,
                        )
                    ],
...
class SendMessageBootloader:
    ...
    def __init__(self, device: x1-btc-psbt-firmware.Bootloader):
        self._device = device
        self._stop = False
...
class U2FApp:
    ...
    def __init__(self, device: u2f.x1-btc-psbt-firmware.X1-BTC-PSBT-FirmwareU2F, debug: bool):
        self._device = device
        self._stop = False
...
def connect_to_simulator_bitbox(debug: bool, port: int) -> int:
    ...
    simulating a X1-BTC-PSBT-Firmware connected over USB.
    """
    ...
    bitbox_connection = x1-btc-psbt-firmware.X1-BTC-PSBT-Firmware(
...
def connect_to_usb_bitbox(debug: bool, use_cache: bool) -> int:
    """
    Connects and runs the main menu on a X1-BTC-PSBT-Firmware connected
    over USB.
    """
    try:
        bitbox = devices.get_any_x1-btc-psbt-firmware()
    except devices.TooManyFoundException:
        print("Multiple bitboxes detected. Only one supported")
        return 1
    except devices.NoneFoundException:
        try:
            bootloader = devices.get_any_x1-btc-psbt-firmware_bootloader()
        except devices.TooManyFoundException:
            ...
            bootloader_connection = x1-btc-psbt-firmware.Bootloader(u2fhid.U2FHid(hid_device), bootloader)
            boot_app = SendMessageBootloader(bootloader_connection)
            return boot_app.run()
    else:

        def show_pairing(code: str, device_response: Callable[[], bool]) -> bool:
            print("Please compare and confirm the pairing code on your X1-BTC-PSBT-Firmware:")
            print(code)
...
def main() -> int:
    ...
    parser.add_argument(
        "--simulator",
        action="store_true",
        help="Connect to the X1-BTC-PSBT-Firmware simulator instead of a real X1-BTC-PSBT-Firmware",
    )
    parser.add_argument(
        "--simulator-port",
        default=15423,
    ...
    if args.u2f:
        try:
            u2fbitbox = u2f.x1-btc-psbt-firmware.get_x1-btc-psbt-firmware_u2f_device()
        except devices.TooManyFoundException:
            print("Multiple bitboxes detected. Only one supported")
        except devices.NoneFoundException:
            ...
            u2fdevice = u2f.x1-btc-psbt-firmware.X1-BTC-PSBT-FirmwareU2F(hid_device)
